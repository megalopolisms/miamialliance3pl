...
async function toolEscalateToHuman(input, customerPhone) {
    ...
    if (twilioClient && input.urgency === 'high') {
        await sendWhatsApp(adminPhone, `🚨 URGENT: Customer ${customerPhone} needs help. Reason: ${input.reason}. Ticket: ${docRef.id}`);
    }
...
async function toolFileClaim(input, customerPhone) {
    ...
    if (input.claimed_amount > 500) {
        const adminPhone = functions.config().admin?.phone || '+13055041323';
        if (twilioClient) {
            await sendWhatsApp(adminPhone, `💰 High-value claim filed: ${claimNumber} - $${input.claimed_amount} - ${input.claim_type} - Tracking: ${input.tracking_number}`);
        }
    }
...
async function generateClaudeResponseWithTools(userMessage, conversationHistory, customerPhone) {
    ...
    try {
        ...
        if (sentiment === 'negative' || sentiment === 'urgent') {
            const adminPhone = functions.config().admin?.phone || '+13055041323';
            if (twilioClient) {
                await sendWhatsApp(adminPhone, `⚠️ ${sentiment.toUpperCase()} customer ${customerPhone}: "${userMessage.slice(0, 100)}..."`);
            }
        }
        ...
        if (escalations.size > 5 && chatLogs.size > 0) {
            ...
            if (rate > 20) {
                ...
                if (adminPhone && twilioClient) {
                    await sendWhatsApp(adminPhone, `⚠️ AI Performance Alert: ${rate.toFixed(1)}% escalation rate yesterday (${escalations.size}/${chatLogs.size} conversations). Review needed.`);
                }
            }
        }
...
async function processFeedbackRating(phoneNumber, rating) {
    ...
    if (normalizedRating.score <= 2) {
        const adminPhone = functions.config().admin?.phone;
        if (adminPhone && twilioClient) {
            await sendWhatsApp(adminPhone, `⚠️ Low feedback score (${normalizedRating.label}) from ${phoneNumber}. Review conversation.`);
        }
    }
...
exports.weeklyCustomerSummary = functions.pubsub
    ...
    .onRun(async (context) => {
        ...
        for (const doc of activeConversations.docs) {
            ...
            if (shipments.size > 0) {
                const message = `📊 Miami Alliance 3PL Weekly Summary:\n\n` +
        ...
        for (const [phone, count] of Object.entries(customerShipments)) {
            if (count >= 10) {
                ...
                if (!prefDoc.exists || !prefDoc.data().vip) {
                    ...
                    if (twilioClient) {
                        await sendWhatsApp(phone, `👑 Congratulations! You've been upgraded to VIP status at Miami Alliance 3PL!\n\nYour benefits:\n✨ Priority handling\n📞 Direct line access\n💰 Preferred rates\n🚀 Same-day processing\n\nThank you for your loyalty!`);
                    }
                    upgradedCount++;
                }
            }
...
async function toolModifyOrder(input, customerPhone) {
    ...
    switch (input.action) {
        ...
        case 'expedite':
            ...
            const adminPhone = functions.config().admin?.phone;
            if (adminPhone && twilioClient) {
                await sendWhatsApp(adminPhone, `🚀 EXPEDITE REQUEST: ${input.tracking_number} - Customer: ${customerPhone}`);
            }
            break;
...
const TRANSLATIONS = {
    es: {
        welcome: "¡Hola! Soy el asistente de IA de Miami Alliance 3PL. ¿Cómo puedo ayudarte hoy?",
        not_found: "No encontré información sobre eso. ¿Puedo ayudarte con algo más?",
        error: "Lo siento, tengo problemas ahora. Por favor intenta de nuevo o llama al (305) 555-3PL1.",
        escalation: "Te he conectado con nuestro equipo. Un agente te contactará pronto.",
        feedback_request: "⭐ ¿Cómo fue tu experiencia con Miami Alliance 3PL hoy?\n\nResponde con un número:\n1️⃣ Excelente\n2️⃣ Bueno\n3️⃣ Regular\n4️⃣ Malo\n5️⃣ Muy malo",
